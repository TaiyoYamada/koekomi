...
### 無音カット（生成音声の頭に空白を作らないため）

録音ボタンを押してから読み始めるまでの「間」を参照音声に残すと、声クローンがその間ごと真似て
生成音声の先頭に無音が入ります（再生でも書き出した動画でもそのまま空白になります）。そこで:

- **参照音声**: `services/audio.py` の `convert_to_wav(..., trim_silence=True)` が ffmpeg の
  `silenceremove` で前後の無音を落とします（頭は 0.1 秒だけ残す）。カット後が 1 秒未満なら
  切りすぎ／ほぼ無音とみなしてカット無しに戻します。ffmpeg が無ければ変換ごとスキップします。
- **生成音声**: `adapters/qwen_tts.py` の `_trim_silence` が保存前に前後の無音を落とします
  （しきい値はピーク比 2%、前後に 0.05 秒だけ残す）。全部無音なら何もしません。
