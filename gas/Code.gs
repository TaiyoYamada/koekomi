 ...
 *
 * やることは1つだけ: **「今日のサーバーURLを配る」**。
 *
 * 以前はここが負荷分散器も兼ねていた（presence / assign / release /
 * activeCount / capacity / assignedCount）。だが端末は最大10人、
 * サーバーは6台（Colab Pro+ 3アカウント × 2タブ）。どこが空いているかは配らない
 * ＝端末側がハッシュで散らすので、名簿は調整に関与しない。
 *
...
function doPost(e) {
  ...
  // 書き込みは 6台 × 2回/分 しか来ないので、待たされることはまず無い。
