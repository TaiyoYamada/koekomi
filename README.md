...
規模: **同時 最大10人 / サーバー6台（Colab Pro+ 3アカウント × 2タブ）**。
...
```mermaid
flowchart TD
    iPad["子どものiPad<br/>(固定URLを開くだけ)"] -->|"① 今日のURL一覧を取得"| GAS["GAS + Google Sheets<br/>(サーバー名簿)"]
    iPad -->|"② 端末IDのハッシュで1台選び<br/>/health が通るまで順に試す"| Colab
    Colab["Colab #1..#6<br/>(FastAPI + Cloudflare Tunnel)"] -->|register / heartbeat| GAS
...
この4つが崩れると、複数台構成が冗長化として機能しなくなります。
...
> **台数は「1台落ちても気づかれない」ためと、一斉に押されたときの待ち時間のためです。**
> 授業45分で平均すれば GPU は 95% 以上遊びますが、**待ち時間を決めるのは平均ではなくバースト**です。
> 1台のGPUは同時に1行しか作らない（`WORKERS=1` + `TTS_SERIALIZE=1`）ので、
> その台に来た総行数 × 1行の秒数が、そのまま最後の子の待ち時間になります。
> 台数を倍にすると1台あたりの行数が半分になり、その待ちも半分になります。
> それでも**負荷分散はしていません**（＝共有状態が要らない）。散らすのは端末IDのハッシュ、
...
## 3. 本番運用（Colab Pro+ 3アカウント × 2タブ = 6台）

`colab/start_backend01〜06.ipynb` を1冊ずつ開いて実行します。`SERVER_ID` / `SERVER_COLOR` は
ノートブックごとに設定済みなので、そのまま実行してください。

| ノートブック             | ID                | color          | 想定アカウント |
| ------------------------ | ----------------- | -------------- | -------------- |
| `start_backend01` / `04` | colab-1 / colab-4 | red / yellow   | アカウントA    |
| `start_backend02` / `05` | colab-2 / colab-5 | blue / purple  | アカウントB    |
| `start_backend03` / `06` | colab-3 / colab-6 | green / orange | アカウントC    |

**同じアカウントで2台動かすときは、必ず別のノートブックを開くこと。**
同じファイルを2タブで開いてもランタイムは共有され、GPU は増えません。

**⚠️ 2つ目のセッションは GPU を取り逃すことがあります。** 取り逃しても Qwen3-TTS は
警告なく CPU で動き、`/health` は `ok`・音声は `qwen` のまま1行が数十秒になります。
見た目が健康なので iPad は普通に割り当て続け、**その台の子だけが待たされます**。
起動ログ `[0/6]` の `GPU:` 行と `[5/6]` の `推論デバイス:` 行を必ず確認してください。
CPU と出た台は使わないこと（名簿から外すにはランタイムを停止すれば `lastSeen` が古くなって自動的に候補から消えます）。
...
動き続け、コンピューティングユニットを消費します。6台ぶん動きっぱなしなら消費も6倍です。
...
| AI実行環境     | Google Colab Pro+ 3アカウント × 2セッション = 6台       |
